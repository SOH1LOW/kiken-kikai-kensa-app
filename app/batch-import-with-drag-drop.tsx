...
export default function BatchImportWithDragDropScreen() {
  const colors = useColors();
  const router = useRouter();
  const dragDropManager = useRef(new DragDropStateManager()).current;
  ...
  const [dragDropState, setDragDropState] = useState<DragDropState>(dragDropManager.getState());
  const [showInvalidFilesAlert, setShowInvalidFilesAlert] = useState(false);
  const [invalidFilesMessage, setInvalidFilesMessage] = useState('');
  ...
  const handleDragOver = (event: React.DragEvent<HTMLDivElement>) => {
    DragDropHandler.handleDragOver(event as any);
    dragDropManager.startDrag(event.dataTransfer?.items?.length || 0);
  };

  const handleDragLeave = (event: React.DragEvent<HTMLDivElement>) => {
    DragDropHandler.handleDragLeave(event as any);
    dragDropManager.endDrag();
  };

  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    const { validFiles, invalidFiles } = DragDropHandler.handleDrop(event as any);

    dragDropManager.endDrag();

    if (invalidFiles.length > 0) {
      const message = DragDropHandler.getValidationErrorMessage(invalidFiles);
      setInvalidFilesMessage(message);
      setShowInvalidFilesAlert(true);
    }
  ...
  // ファイルをバッチに追加
  const addFilesToBatch = (newFiles: BatchFile[]) => {
    ...
    Alert.alert('成功', `${newFiles.length}個のファイルを追加しました`);
  };
        ...
        <View className="mb-2 flex-row items-center justify-between">
          <View className="flex-1">
            ...
            <Text className="text-xs text-muted">
              {DragDropHandler.formatFileSize(item.size)} • {item.type.toUpperCase()}
            </Text>
          </View>
          <Pressable
    ...
    <View className="mb-3 rounded-lg border border-border bg-surface p-4">
      ...
      {item.success ? (
        <View className="gap-1">
          <Text className="text-sm text-foreground">📝 {item.questions.length}問抽出</Text>
  ...
  return (
    <ScreenContainer className="p-4">
        ...
        <View className="gap-4">
          ...
          >
            <View className="items-center gap-3">
              ...
              <Text className="text-center text-xs text-muted">
                {DragDropHandler.getSupportedFormatsDescription()}
              </Text>
            </View>
          </View>
        ...
        <View className="flex-1 bg-black/50">
          <View className="mt-auto rounded-t-2xl bg-background p-4">
            ...
            <View className="flex-row gap-2">
              ...
              <Pressable
                onPress={() => handleSaveAnswerKey(currentAnswerKeyInput)}
      ...
      {/* 無効なファイルアラート */}
      <Modal visible={showInvalidFilesAlert} animationType="fade" transparent>
        <View className="flex-1 items-center justify-center bg-black/50">
          <View className="w-11/12 rounded-lg bg-background p-4">
            <Text className="mb-3 text-lg font-bold text-foreground">ファイル処理エラー</Text>
            <Text className="mb-4 text-sm text-muted">{invalidFilesMessage}</Text>
            <Pressable
              onPress={() => setShowInvalidFilesAlert(false)}
